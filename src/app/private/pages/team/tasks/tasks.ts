...
export class TeamTasks implements OnInit, OnDestroy {
    ...
    async ngOnInit() {
        this.breadcrumbService.setTitle('Team Tasks');
    ...
    async seedDemoTask() {
        const ws = await this.authService.getCurrentWorkspace();
        if (ws) {
            await this.taskService.create(ws.id, ws.name, 'Finalize Collaborative Sync Architecture');
        }
    }
}
